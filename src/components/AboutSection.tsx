    ...
    <section id="about" className="py-20 bg-white text-forest-950 relative overflow-hidden border-b border-forest-100/50">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start text-left">
          ...
          <div className="lg:col-span-7 bg-forest-50/40 p-8 rounded-3xl border border-forest-100 relative shadow-sm">
            <div className="flex items-center gap-3 mb-6">
              <Milestone className="w-6 h-6 text-forest-700" />
              <div>
                <h3 className="font-serif text-2xl font-bold text-forest-905">
                  {SIGNATURE_ITINERARY.title}
                </h3>
                <p className="text-xs text-forest-600 uppercase tracking-widest font-mono font-bold">
                  The Complete Primate Expedition
                </p>
              </div>
            </div>

            <div className="relative border-l border-forest-200/80 ml-4 space-y-8 py-2">
              {SIGNATURE_ITINERARY.steps.map((step, idx) => (
                <div key={idx} className="relative pl-8 group">
                  {/* Timeline Dot Indicator */}
                  <div className="absolute left-0 -translate-x-1/2 top-1.5 w-3 h-3 bg-forest-600 rounded-full border-2 border-white group-hover:bg-forest-800 transition-colors duration-300"></div>
                  
                  <div className="flex flex-col md:flex-row md:items-baseline md:justify-between gap-1.5">
                    <span className="font-mono text-[10px] font-bold text-forest-800 uppercase bg-emerald-50 px-2 py-0.5 rounded border border-forest-100 inline-block max-w-fit">
                      {step.days}
                    </span>
            ...
            <div className="mt-8 pt-6 border-t border-forest-100/80 flex items-center justify-between flex-wrap gap-4">
              <div className="flex items-center gap-2.5 text-xs text-forest-700">
                <Heart className="w-4 h-4 text-forest-600 fill-forest-600/10 animate-pulse" />
                <span>Private Customized Itineraries Available</span>
              </div>
              <button 
                onClick={() => onNavigate('booking')}
                className="text-xs text-forest-800 hover:text-forest-950 font-bold underline uppercase tracking-widest transition cursor-pointer text-left font-mono"
              >
